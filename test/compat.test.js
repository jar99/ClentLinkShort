...
const GOLDEN = [
  ["wHnClZjdg", "https://example.com/"],
  ["IABdTkSjtChe4-NiMw", "https://github.com/anthropics/claude-code"],
  ["wZKDwsbCJ8GzStxcZ6ItQoJaTBxjxRmXgKPozWZmA",
  ...
  ["wxmksGcFe1fDF8Ahtjrhk8Ghg", "https://blog.startup.io/posts/2026-hello"],
  ["xSceDE49IvgDgcBVGw", "http://old-site.net/index.php?id=42"],
  ["CJXfe1e4DY5ZvQWoUTYCqYPMbO4H7CuQyTM",
    "mailto:someone@example.com?subject=Hi%20there"],
  ["wHnClZj3oiigFDgbrZX4vyG4ksbA", "https://example.co.uk/a/b/c?q=test#frag"],
  ["CEQu9pzFGN7gNjlm9BahRML1pJls", "https://user:pw@example.com/secret"],
  ["wHnClZjcVM54eziBhNgwbQMJsGC2DCcA8MGE2DBbBhNgwcEGE2DBwwYTYMF3A3WzCW4C26C0-w",
...
test("the frozen tables are byte-for-byte what v1 shipped", () => {
  ...
  assert.equal(hash([...HOST_CODE_LENGTHS]), "4c5b7e00de8392ed", "HOST_CODE_LENGTHS changed");
  assert.equal(hash([...SUFFIXES]), "be6e4dd923538945", "SUFFIXES changed");
});
...
test("the indexed tables only ever grow", () => {
  ...
  assert.ok(TEMPLATES.length >= 86);
  assert.equal(hash(TEMPLATES.slice(0, 86).map((t) => [t.pattern, ...t.slots])),
    "34eb4576f8511040", "TEMPLATES prefix changed");
